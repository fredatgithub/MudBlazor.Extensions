...
namespace MudBlazor.Extensions.Explorer.Services
{
    ...
    {
        public static string GetPackageName(string packageUrl)
        {
            var packageName = packageUrl.Replace("https://www.nuget.org/packages/", "").TrimEnd('/');
            return packageName;
        }

        public static async Task<string> GetLatestVersion(string packageUrl)
        {
        ...
        public static async Task<string> GetIconUrl(string packageUrl)
        {
